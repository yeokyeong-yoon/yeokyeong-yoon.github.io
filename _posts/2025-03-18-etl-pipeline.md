...
| 구분               | 내용                                                         |
|--------------------|--------------------------------------------------------------|
| **기본 파이프라인 구축** | - Airflow에서 Databricks로 워크플로우 마이그레이션 완료<br>- 테이블별 독립 처리를 통한 OOM 문제 해결<br>- 기본적인 에러 로깅 구현 |
| **실제 적용 중인 기능** | - 압축 파일 해제 및 S3 업로드<br>- 테이블별 병합 작업 수행<br>- 기본적인 파일 수 검증 |

### 8.2 개선이 필요한 부분

| 구분               | 내용                                                         |
|--------------------|--------------------------------------------------------------|
| **아키텍처 관점**     | - 현재는 모놀리식에 가까운 구조<br>- 테이블 간 의존성 처리 로직 부재<br>- 장애 복구 전략 미흡 |
| **성능 관점**        | - Spark 튜닝 최적화 필요<br>- 리소스 사용량 모니터링 체계 부재<br>- 병렬 처리 효율 개선 필요 |

### 8.3 향후 개선 계획

| 구분               | 내용                                                         |
|--------------------|--------------------------------------------------------------|
| **아키텍처 개선**     | - 마이크로서비스 아키텍처 도입 검토<br>- 이벤트 기반 아키텍처로 전환<br>- 멀티 테넌시 지원을 위한 구조 개선 |
| **모니터링 강화**    | - Datadog 기반 모니터링 대시보드 구축<br>- SLO/SLA 정의 및 모니터링<br>- 알림 체계 고도화 |
| **데이터 품질 관리**  | - 자동화된 데이터 검증 파이프라인 구축<br>- 품질 메트릭 정의 및 측정<br>- 실시간 이상 감지 시스템 도입 |
| **개발 프로세스 개선** | - CI/CD 파이프라인 구축<br>- 테스트 자동화<br>- 코드 품질 관리 도구 도입 |

## 9. SWE 관점에서의 교훈

### 9.1 기술적 교훈

지금까지의 경험에서 도출된 주요 문제점은 다음과 같습니다:
- 초기 설계 단계에서 확장성을 충분히 고려하지 못했습니다.
- 테스트 코드가 부족하여 시스템의 신뢰성이 저하되었습니다.
- 모니터링 및 알림 체계가 미비하여 문제 발생 시 대응이 어려웠습니다.

향후 개선 방안은 다음과 같습니다:
- 설계 단계부터 확장성을 염두에 두고 진행합니다.
- TDD(테스트 주도 개발) 기반의 개발 프로세스를 도입합니다.
- 체계적인 코드 리뷰 프로세스를 수립하여 품질을 높입니다.

### 9.2 협업 관점의 교훈

현재 협업 상태는 다음과 같습니다:
- ML 팀과의 커뮤니케이션이 비정기적으로 이루어지고 있습니다.
- 문서화가 부족하여 지식 공유에 어려움이 있습니다.

개선 계획은 다음과 같습니다:
- API 계약 기반의 협업 체계를 수립합니다.
- 기술 문서화 프로세스를 정립하여 정보의 일관성을 높입니다.
- 정기적인 크로스팀 리뷰 세션을 운영하여 협업을 강화합니다.

## 10. 결론

이번 프로젝트를 통해 SWE로서 대규모 데이터 처리 시스템을 설계하고 구현하는 과정에서 많은 교훈을 얻었습니다. 특히 초기 설계의 중요성과 점진적인 개선의 필요성을 깊이 체감했습니다.
