...
작년에 "다이나믹 프라이싱 서비스"가 런칭되었고, 올해 저는 파이프라인 구축 담당으로 투입되었습니다. 현재는 해외 파트너사 1곳의 데이터만 처리하고 있지만, 곧 국내 1곳과 해외 1곳이 추가될 예정입니다. 각 파트너사마다 데이터 스키마와 포맷이 상이하여, 이를 효율적으로 통합 처리할 수 있는 표준화된 파이프라인이 필요했습니다.
