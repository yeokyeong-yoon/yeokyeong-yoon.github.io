...
가장 인상 깊었던 순간은 40,000개의 파일이 처음으로 성공적으로 처리되어 각 테이블별 CSV로 변환되었을 때였습니다. 그 때의 스크린샷을 아직도 간직하고 있을 정도입니다. 그 전까지는 정말 다양한 실패를 경험했었습니다. 작업은 다 완료된 것처럼 보이는데 스레드가 종료되지 않아 워크플로우가 몇 시간씩 돌아가는 상황이라든지, 메모리 부족으로 클러스터가 갑자기 다운되는 상황이라든지... 수십 번의 이런 실패와 디버깅 끝에 모든 테이블이 한 번에 성공적으로 처리되는 모습을 봤을 때의 희열은 말로 표현하기 어려울 정도였습니다. 
