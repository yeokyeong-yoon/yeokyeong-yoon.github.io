...
### 5.1 데이터 품질 관리의 중요성
ETL 파이프라인 구축 초기에는 데이터 이동과 변환에 집중했지만, 실제로는 데이터 품질 관리가 더 큰 과제였다. 데이터의 정확성과 일관성을 보장하기 위한 검증 과정이 예상보다 많은 시간과 노력을 필요로 했다.

### 5.2 확장성과 실용성의 균형
향후 확장을 고려한 설계가 생각보다 어려웠다. 너무 많은 추상화와 유연성을 추구하다 보니 코드가 복잡해지고 유지보수가 어려워졌다. 실제로 필요한 기능에 집중하고, 확실한 요구사항만 미리 구현하는 것이 더 효율적이었다.

### 5.3 데이터 분석의 가치
ETL 파이프라인을 통해 데이터의 흐름을 파악하면서, 비즈니스 규칙과 패턴을 자연스럽게 이해하게 되었다. 기술적인 작업이지만, 비즈니스 인사이트를 얻을 수 있는 좋은 기회였다.
