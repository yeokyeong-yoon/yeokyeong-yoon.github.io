...
시스템은 크게 두 개의 EKS 클러스터로 구성됩니다: Data Team이 관리하는 관리용 클러스터와 여러 서비스가 공유하는 공용 클러스터입니다. 각 클러스터의 역할과 컴포넌트 간의 상호작용은 다음과 같습니다:

### 2.1 Data Team EKS 클러스터
...
폴링 주기를 10초로 설정한 것은 다음과 같은 고려사항을 바탕으로 했습니다:
...
2. **시스템 부하와 리소스 효율성**
   ...
   - 실제로는 1-3초 정도로 설정해도 충분했을 수 있음
   - 하지만 현재 사용 사례에서는 10초로도 충분한 응답성 제공

3. **유연한 설정**
   - FeatureFlagManager는 폴링 주기를 설정 가능하도록 설계됨
   - 필요에 따라 더 짧은 주기(예: 1-3초)로 조정 가능
   - A/B 테스트나 실시간 기능 제어가 필요한 경우 짧은 주기 사용 가능

### 2.3.2.1 대안적 접근 방식 검토
...
이러한 하이브리드 구조는 Kubernetes 환경에서의 안정적인 운영과 높은 성능을 모두 고려한 설계입니다.
