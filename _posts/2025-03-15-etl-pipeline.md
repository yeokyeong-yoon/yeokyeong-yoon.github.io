...
특히 리소스 최적화와 관련된 중요한 교훈을 얻었습니다. 초기에는 단순히 클러스터 리소스를 최대화하면 성능이 향상될 것이라 가정했습니다. 그러나 실제 테스트 결과, 리소스 증가가 성능 향상으로 직결되지 않음을 확인했습니다. 이는 Spark의 특성상 데이터 처리 방식과 파티셔닝 전략이 리소스 할당보다 더 중요한 요소임을 시사합니다. 이를 통해 단순한 리소스 투입보다는 데이터 특성과 작업 패턴에 맞는 최적화가 필수적이라는 점을 배웠습니다.
...
SWE로서 이번 프로젝트는 분산 시스템을 실제로 경험할 수 있는 귀중한 기회였습니다. 단일 서버 애플리케이션과는 다른 도전 과제들을 해결하며 분산 컴퓨팅에 대한 실질적인 이해를 넓힐 수 있었습니다.
