...
현재 데이터 수집(Data Ingestion) 파이프라인에서 직면한 주요 과제는 다음과 같습니다:

1. **대규모 데이터 소스 처리의 한계**
   - 수만 개의 `.tar.gz` 파일과 CSV 파일을 처리해야 함
   - 단일 프로세스로는 메모리 부족 문제 발생
   - 파일 처리 시간이 너무 김

2. **데이터 품질 관리의 어려움**
   - 다양한 데이터 소스(파트너사)의 데이터 형식 처리 필요
   - 데이터 검증 프로세스 부재
   - 스키마 관리 체계 부재

3. **확장성 문제**
   - 새로운 데이터 소스 추가 시 파이프라인 수정 필요
   - 리소스 사용 효율성 저하
   - 모니터링 시스템 부재

이러한 문제들을 해결하기 위해 Databricks를 활용한 분산 처리 아키텍처를 도입하게 되었습니다.

## 2. 개요 및 배경

작년에 저희 팀은 여행 예약 플랫폼에 다이나믹 프라이싱 서비스를 런칭했고, 올해 저는 데이터 수집(Data Ingestion) 파이프라인 구축 담당으로 투입되었습니다. 다이나믹 프라이싱이란 시장 상황, 수요 변화, 경쟁사 가격 등 다양한 요소를 실시간으로 분석하여 최적의 가격을 자동으로 책정하는 시스템입니다. 현재는 해외 파트너사 하나의 데이터만 처리하고 있지만, 곧 국내외 파트너사가 추가로 합류할 예정입니다. 문제는 각 파트너마다 데이터 구조와 형식이 제각각이라, 이를 효율적으로 통합 처리할 수 있는 표준화된 데이터 수집 파이프라인이 필요했습니다.
...
마지막으로, 모듈화의 가치를 배웠습니다. 처음에는 모든 기능을 하나의 큰 스크립트로 작성했다가 작업을 분리하고 모듈화하니 코드의 가독성과 유지보수성이 크게 향상되었습니다. 각 모듈이 하나의 책임만 가지도록 설계하고 모듈 간 의존성을 최소화하는 것이 시스템의 안정성을 높이는 핵심이었습니다.
...
이번 프로젝트에서는 데이터 파이프라인의 첫 단계인 데이터 수집(Data Ingestion) 부분만을 구현했습니다. 현재까지 달성한 주요 성과는 다음과 같습니다:
...
하지만 전체 데이터 파이프라인 관점에서 아직 해결해야 할 과제들이 더 많아 보입니다:

1. **데이터 변환(Transformation) 파이프라인**
   - 데이터 정규화 및 표준화 프로세스 구축
   - 데이터 품질 검증 강화
   - 스키마 관리 시스템 구축

2. **데이터 적재(Loading) 파이프라인**
   - Delta Lake 도입 및 최적화
   - 데이터 웨어하우스 구축
   - 데이터 접근 제어 시스템 구현
...
특히 새로운 파트너사들의 데이터가 어떤 형태일지 모르는 상황이라 걱정이 됩니다. Data Engineering 관련 책도 더 읽고, 컨퍼런스도 찾아보면서 공부해야겠다는 생각이 듭니다. 일단은 데이터 수집 파이프라인이 동작한다는 것에 의의를 두고, 차근차근 나머지 부분들을 개선해나가면서 더 배워나가야겠습니다.
