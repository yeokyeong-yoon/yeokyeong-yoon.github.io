...
### 1.1 다이나믹 프라이싱 서비스 소개

저는 글로벌 여행 예약 플랫폼의 다이나믹 프라이싱 데이터 솔루션 개발팀에서 근무하고 있습니다. 저희 팀이 운영하는 다이나믹 프라이싱 시스템은 B2B 기반의 혁신적인 서비스로, 실시간 시장 상황과 경쟁사 가격을 분석하여 최적의 가격을 결정합니다. 현재는 소수의 파트너사와만 협력하고 있으나, 앞으로는 더 많은 파트너사들과 함께 서비스 영역을 확장할 계획입니다.
...
이런 다양한 데이터는 각기 다른 형식과 구조를 가지고 있어서, 이를 ML 모델이 학습할 수 있는 형태로 변환하는 과정이 필요합니다. 바로 이 작업을 자동화하고 효율적으로 처리하기 위해 ETL(Extract, Transform, Load) 파이프라인을 구축하게 되었습니다.
