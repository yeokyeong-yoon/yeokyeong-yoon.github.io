...
- **장기적 고려사항**: 클라우드 환경에서는 S3와 같은 객체 저장소를 활용하는 것이 확장성과 안정성 측면에서 유리합니다.
...
- **장기적 고려사항**: 데이터 볼륨이 증가할수록 분산 처리는 필수적입니다. 단일 프로세스는 개발 속도는 빠르지만, 데이터 성장에 대응할 수 없습니다.
...
- **장기적 고려사항**: 모듈화는 확장성과 유지보수성을 높입니다. 데이터 파이프라인의 성장 단계에 따라 적절한 수준의 모듈화가 필요합니다.
...
- **직접 데이터 전달 vs S3 경로 기반 연결**:
  - 직접 데이터 전달: 빠른 데이터 전송, 메모리 효율성이 있지만 작업 간 강한 결합, 장애 복구 어려움
  - S3 경로 기반 연결: 작업 독립성, 장애 복구 용이, 확장성의 장점이 있지만 추가 I/O 오버헤드, 지연 시간 증가
...
특히 인상 깊었던 것은 이론과 실제의 차이를 실감한 순간이었습니다. 처음에는 클러스터 리소스를 최대로 할당하면 더 빠른 처리가 가능할 것이라 생각했지만, 실제로는 그다지 큰 차이를 느끼지 못했습니다. 그때 "아, 이래서 Spark 최적화 기법들이 필요한 거구나"라는 생각이 들었습니다. 단순한 리소스만 투입하는 것이 아니라 실제 데이터와 작업 특성에 맞게 최적화하는 것이 중요하다는 걸 체감했습니다.
...
- **스토리지 최적화**
  ...
  - **장기적 고려사항**: 데이터 파이프라인이 성장함에 따라, 최적화된 저장 형식은 스토리지 비용과 쿼리 성능 측면에서 필수적입니다.
...
- **ETL 로직 표준화**
  ...
  - **장기적 고려사항**: 파이프라인이 성장함에 따라 일반화된 로직이 장기적으로 유지보수성과 확장성을 높입니다.

- **작업 모니터링 간소화**
  - 각 작업의 성공/실패 여부를 더 직관적으로 확인할 수 있는 방법이 필요합니다.
  - 지금은 여러 로그를 찾아보며 확인해야 해서 번거로운 면이 있습니다.
  - **트레이드오프**: 수동 모니터링 vs 자동화된 대시보드
    - 수동 모니터링: 구현 간단, 유연성이지만 시간 소모, 일관성 부족
    - 자동화된 대시보드: 실시간 가시성, 알림, 트렌드 분석의 장점이 있지만 구현 복잡성, 추가 리소스 필요
  - **장기적 고려사항**: 운영 효율성과 문제 조기 감지를 위해 자동화된 모니터링은 필수적입니다.

- **확장성 전략**
  - 데이터 볼륨 증가에 대응하기 위한 확장성 전략을 구체화할 계획입니다.
  - **트레이드오프**: 수동 스케일링 vs 오토스케일링
    - 수동 스케일링: 예측 가능한 비용, 설정 간단하지만 리소스 낭비 또는 부족
    - 오토스케일링: 비용 최적화, 워크로드 변동 대응의 장점이 있지만 구현 복잡성, 예측 불가능한 비용
  - **장기적 고려사항**: 데이터 볼륨과 처리 요구사항은 시간에 따라 변동합니다. 오토스케일링은 비용 효율성과 성능 균형을 맞추는 핵심 전략입니다.

개인적으로는 SWE로서 이 프로젝트를 통해 데이터 엔지니어링에 대한 실전적인 이해를 넓힐 수 있었습니다. 애플리케이션 개발과는 또 다른 문제 해결 접근법이 필요하다는 점이 흥미로웠고, 빅데이터 처리의 특성을 직접 경험하며 엔지니어로서의 시야를 넓힐 수 있었습니다.
...
- **완벽함을 처음부터 추구하지 말자**
  ...
  - **장기적 고려사항**: 데이터 파이프라인은 점진적으로 발전하는 생명체입니다. 초기에는 단순하게 시작하고, 데이터와 요구사항이 성장함에 따라 최적화하고 확장하는 전략이 효과적입니다.
...
- **실패를 두려워하지 말 것**
  ...
  - **장기적 고려사항**: 분산 시스템에서는 실패가 예상되는 이벤트입니다. 장애 복구 메커니즘을 설계 단계부터 고려하는 것이 중요합니다.

- **문서화의 중요성**
  - 여러 번의 시행착오를 겪으면서 문서화의 필요성을 절실히 느꼈습니다.
  - 어떤 시도를 했고, 무엇이 효과적이었으며, 어떤 부분에서 문제가 발생했는지를 기록하다 보니 개념을 더 쉽게 이해할 수 있었습니다.
  - **장기적 고려사항**: 데이터 파이프라인은 복잡한 시스템이므로, 설계 결정, 트레이드오프, 문제 해결 과정을 문서화하는 것이 팀 협업과 지식 공유에 필수적입니다.

- **분리와 모듈화의 중요성**
  - 초기에는 모든 기능을 하나의 큰 스크립트로 작성했다가 디버깅과 유지보수가 어려워졌습니다.
  - 작업을 분리하고 모듈화하면서 코드의 가독성과 유지보수성이 크게 향상되었습니다.
  - **장기적 고려사항**: 모듈화는 데이터 파이프라인의 확장성과 유지보수성을 높입니다. 각 모듈은 단일 책임을 가지고, 모듈 간 의존성을 최소화하는 것이 중요합니다.
