...
describe("DirStore", () => {
  ...
  async function createPromptFile(name: string, content: string = "test source") {
  ...
  describe("list", () => {
    it("should list prompts in root directory with versions", async () => {
      ...
      const prompts = result.prompts.sort((a, b) => a.name.localeCompare(b.name));
  ...
  describe("listPartials", () => {
    it("should list partial prompts with versions", async () => {
      ...
      const partials = result.partials.sort((a, b) => a.name.localeCompare(b.name));
  ...
  describe("load", () => {
    ...
    it("should throw error when requested version does not match", async () => {
      ...
      await expect(store.load("test", { version: "wrongversion" })).rejects.toThrow(
        /Version mismatch/
      );
    });
  });
  ...
  describe("loadPartial", () => {
    ...
    it("should throw error when requested partial version does not match", async () => {
      ...
      await expect(store.loadPartial("test", { version: "wrongversion" })).rejects.toThrow(
        /Version mismatch/
      );
    });
  });
  ...
  describe("save", () => {
    it("should save a prompt and generate correct version", async () => {
      ...
      const saved = await fs.readFile(path.join(tempDir, "test.prompt"), "utf-8");
    ...
    it("should save a prompt with variant and generate correct version", async () => {
      ...
      await store.save(prompt);
      const saved = await fs.readFile(path.join(tempDir, "test.v1.prompt"), "utf-8");
      expect(saved).toBe(source);
    ...
    it("should save a prompt in subdirectory and generate correct version", async () => {
      ...
      await store.save(prompt);
      const saved = await fs.readFile(path.join(tempDir, "subdir/test.prompt"), "utf-8");
      expect(saved).toBe(source);
  ...
  describe("delete", () => {
    it("should delete a prompt", async () => {
      ...
      await expect(fs.access(path.join(tempDir, "test.prompt"))).rejects.toThrow();
    });
    ...
    it("should delete a prompt with variant", async () => {
      ...
      await expect(fs.access(path.join(tempDir, "test.v1.prompt"))).rejects.toThrow();
    });
