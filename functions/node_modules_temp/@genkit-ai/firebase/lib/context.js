...
async function parseAuth(authHeader) {
...
async function parseAppCheck(token, consumeAppCheckToken) {
  if (debugSkipTokenVerification()) {
    const decoded = unsafeDecodeToken(token);
    return {
      appId: decoded["sub"],
  ...
  try {
    return await (0, import_app_check.getAppCheck)().verifyToken(token, {
